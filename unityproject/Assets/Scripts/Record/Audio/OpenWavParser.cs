...
public static class OpenWavParser
{
                ...
                for (int i = 0; i < _sampleCount; i++)
                {
                    ...
                    switch(_bitsPerSample)
                    {
                        case Resolution._16bit:
                            ...
                            {
                                Debug.LogWarning(wavFile.Length +","+ i +","+ sampleIndex);
    ...
    public static byte[] NewAudioClipDataToByteArray(float[] samples, int samplesNum, int channels, int frequency, Resolution res = Resolution._16bit)
    {
        ...
        writer.Write(16); // Format chunk size (16 bytes)
        writer.Write((short)1); // Format (PCM)
        writer.Write((short)channels); // Channel count
        writer.Write(frequency); // Sample rate
        writer.Write(frequency * channels * 2); // Byte rate
        writer.Write((short)(channels * 2)); // Block align
        writer.Write((short)16); // Bits per sample
        writer.Write("data".ToCharArray());
        writer.Write(samplesNum * 2); // Data chunk size
        ...
        // Calculate and write the data size
        int dataSize = (int)stream.Length - 8;
        stream.Seek(4, SeekOrigin.Begin);
        writer.Write(dataSize);

        // Return WAV data as byte array
    ...
    ///<summary>Returns a wav file from any AudioClip Data (modified for async call)</summary
    public static byte[] AudioClipDataToByteArray(float[] samples, int channels, int frequency, Resolution res = Resolution._16bit)
    {   
        ...
        wavFile.AddRange(System.BitConverter.GetBytes((ushort)channels));                  // NumChannels
        wavFile.AddRange(System.BitConverter.GetBytes(frequency));                         // SampleRate
        wavFile.AddRange(System.BitConverter.GetBytes(frequency * channels * size));  // ByteRate
        wavFile.AddRange(System.BitConverter.GetBytes((ushort)(channels * size)));         // BlockAlign
